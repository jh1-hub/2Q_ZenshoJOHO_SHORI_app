...
const getTermIcon = (term: string, size: number = 32) => {
  ...
  else if (t.includes('sns') || t.includes('メール') || t.includes('メーリング') || t.includes('コミュニケーション')) IconComponent = MessageSquare;
  ...
  else if (t.includes('通信') || t.includes('ict') || t.includes('ネットワーク') || t.includes('lan') || t.includes('wan') || t.includes('パケット') || t.includes('プロトコル') || t.includes('ip') || t.includes('アドレス')) IconComponent = Network;
  ...
  else if (t.includes('詐欺') || t.includes('有害') || t.includes('攻撃') || t.includes('マルウェア') || t.includes('ウイルス') || t.includes('脅威') || t.includes('不正')) IconComponent = AlertTriangle;
  else if (t.includes('id') || t.includes('パスワード') || t.includes('生体') || t.includes('顔') || t.includes('虹彩') || t.includes('静脈') || t.includes('網膜') || t.includes('声紋')) IconComponent = Fingerprint;
  else if (t.includes('認証')) IconComponent = Key;
  else if (t.includes('法') || t.includes('権利') || t.includes('知的財産') || t.includes('特許') || t.includes('実用新案') || t.includes('意匠') || t.includes('商標')) IconComponent = Gavel;
  else if (t.includes('著作権') || t.includes('公表権') || t.includes('氏名表示') || t.includes('同一性保持') || t.includes('上演') || t.includes('譲渡') || t.includes('貸与') || t.includes('複製')) IconComponent = Copyright;
  else if (t.includes('クリエイティブ')) IconComponent = CreativeCommons;
  else if (t.includes('ハードウェア') || t.includes('装置') || t.includes('cpu') || t.includes('コア') || t.includes('プロセッサ')) IconComponent = Cpu;
  else if (t.includes('記憶') || t.includes('メモリ') || t.includes('hdd') || t.includes('ssd') || t.includes('ディスク') || t.includes('ドライブ') || t.includes('cd') || t.includes('dvd') || t.includes('bd')) IconComponent = HardDrive;
  else if (t.includes('解像度') || t.includes('カメラ')) IconComponent = Camera;
  else if (t.includes('bluetooth') || t.includes('無線') || t.includes('wifi') || t.includes('wi-fi') || t.includes('ssid')) IconComponent = Wifi;
  else if (t.includes('アクセスポイント')) IconComponent = Radio;
  else if (t.includes('hdmi') || t.includes('usb') || t.includes('ケーブル')) IconComponent = Link;
  else if (t.includes('デジタル') || t.includes('アナログ') || t.includes('ビット') || t.includes('バイト') || t.includes('2進数') || t.includes('16進数') || t.includes('バイナリ') || t.includes('コード')) IconComponent = Binary;
  else if (t.includes('変換') || t.includes('圧縮') || t.includes('解凍') || t.includes('zip')) IconComponent = RefreshCw;
  else if (t.includes('ms') || t.includes('μs') || t.includes('ns') || t.includes('ps') || t.includes('fs') || t.includes('時間') || t.includes('時計') || t.includes('リアルタイム') || t.includes('バッチ')) IconComponent = Clock;
  else if (t.includes('ソフトウェア') || t.includes('os') || t.includes('アプリ') || t.includes('システム')) IconComponent = Settings;
  ...
  else if (t.includes('ファイル') || t.includes('テキスト') || t.includes('csv') || t.includes('pdf')) IconComponent = FileText;
  else if (t.includes('フォルダ') || t.includes('ディレクトリ')) IconComponent = Archive;
  else if (t.includes('インターネット') || t.includes('プロバイダ') || t.includes('web') || t.includes('ブラウザ') || t.includes('url') || t.includes('html') || t.includes('http')) IconComponent = Globe;
  ...
  else if (t.includes('セキュリティ') || t.includes('暗号') || t.includes('盾') || t.includes('ファイアウォール') || t.includes('vpn') || t.includes('ssl')) IconComponent = Shield;
  ...
  else if (t.includes('シミュレーション') || t.includes('プレゼンテーション') || t.includes('画面')) IconComponent = Monitor;
  else if (t.includes('アルゴリズム') || t.includes('プログラム') || t.includes('流れ図')) IconComponent = Terminal;
  else if (t.includes('ブック') || t.includes('ワークシート') || t.includes('セル') || t.includes('行') || t.includes('列') || t.includes('表')) IconComponent = Table;
  else if (t.includes('関数') || t.includes('演算子') || t.includes('計算') || t.includes('sum') || t.includes('average') || t.includes('max') || t.includes('min') || t.includes('count') || t.includes('round') || t.includes('if') || t.includes('vlookup')) IconComponent = Calculator;
...
export default function App() {
  ...
  const takeScreenshot = () => {
    ...
    // レベルに応じたカラーパレットの定義
    let color1 = '#374151'; // デフォルト（Lv 1-5）: Gray 700
    let color2 = '#1f2937'; // Gray 800
    let accentColor = '#60a5fa'; // Blue 400

    if (level >= 31) {
      color1 = '#111827'; // Legend: Gray 900
      color2 = '#030712'; // Gray 950
      accentColor = '#38bdf8'; // Sky 400
    } else if (level >= 21) {
      color1 = '#1e293b'; // Grandmaster: Slate 800
      color2 = '#0f172a'; // Slate 900
      accentColor = '#818cf8'; // Indigo 400
    } else if (level >= 16) {
      color1 = '#3f3f46'; // Master: Zinc 700
      color2 = '#27272a'; // Zinc 800
      accentColor = '#a78bfa'; // Violet 400
    } else if (level >= 11) {
      color1 = '#4b5563'; // Expert: Gray 600
      color2 = '#374151'; // Gray 700
      accentColor = '#94a3b8'; // Slate 400
    } else if (level >= 6) {
      color1 = '#64748b'; // Apprentice: Slate 500
      color2 = '#475569'; // Slate 600
    ...
    ctx.fillText('IT QUIZ STATS 2', 960, 540);
    ...
    ctx.fillText('IT QUIZ MASTER 2', 1870, 1000);
  ...
  const calculateLevel = (collection: Record<string, number>) => {
    ...
    // Max points = 243 * 3 = 729
    // Level = 1 + floor(98 * (points / 729)^0.68)
    // Exponent 0.68 ensures 1 point = Level 2
    if (totalPoints === 0) return 1;
    const level = 1 + Math.floor(98 * Math.pow(totalPoints / 729, 0.68));
  ...
  const userLevelProgress = useMemo(() => {
    ...
    const getPointsForLevel = (L: number): number => {
      if (L <= 1) return 0;
      return Math.ceil(729 * Math.pow((L - 1) / 98, 1 / 0.68));
    };
  ...
  const getCategoryColor = (categoryId: string) => {
    switch (categoryId) {
      case '1_3_5': return { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-100', accent: 'bg-emerald-500', light: 'bg-emerald-100/50' };
        ...
        <header className="sticky top-0 z-40 bg-theme-bg/80 backdrop-blur-md border-b border-theme-border">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 md:h-20 flex items-center justify-between">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <h1 className="text-lg md:text-2xl font-theme-heading font-bold tracking-tight">IT Quiz Master 2</h1>
        ...
        {gameState === 'START' && (
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl font-theme-heading font-bold mb-4 tracking-tight">
              IT Quiz <span className="italic text-theme-accent">Master 2</span>
              <span className="block text-2xl md:text-3xl mt-2 font-sans font-medium text-theme-text-muted">【情報処理】の知識を極めよう</span>
            </h1>
